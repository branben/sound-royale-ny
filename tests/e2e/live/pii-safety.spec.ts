...
test.describe('PII Safety', () => {
  ...
  test('player_secret is never exposed in console logs or network traffic', async ({ page, request }) => {
    ...
    const secretInConsole = consoleMessages.filter(
      (msg) => msg.includes(playerSecret),
    );
    expect(secretInConsole, `Secret found in console: ${secretInConsole.join(', ')}`).toHaveLength(0);
    ...
    const errorElements = await page.locator('[data-testid="error-message"], .error, .toast-error').all();
