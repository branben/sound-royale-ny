...
interface CustomTooltipProps {
  active?: boolean;
  payload?: readonly TooltipPayload<number, string>[];
}
...
export const PlayerStatsRadar: React.FC<PlayerStatsRadarProps> = ({
  player,
  genrePerformance,
  roomGenres,
}) => {
  ...
  if (roomGenres && roomGenres.length > 0) {
    ...
    const performanceMap = new Map(genrePerformance.map((g) => [g.genre.toLowerCase(), g]));
    ...
    const chartData = uniqueRoomGenres.slice(0, 9).map((genreLower) => {
      ...
      const lossesPercent =
        perf && perf.total_rounds > 0
          ? ((perf.total_rounds - perf.wins) / perf.total_rounds) * 100
          : 0;
      return {
          ...
          <ChartContainer config={chartConfig} className="aspect-square w-full">
            <ResponsiveContainer width="100%" height={300}>
              <RadarChart data={chartData}>
                ...
                <PolarAngleAxis dataKey="axis" tick={{ fill: '#fff', fontSize: 10 }} />
                <PolarRadiusAxis
                  angle={90}
                  domain={[0, 100]}
  ...
  const chartData = selectedGenres.map((genre) => {
    ...
    const lossesPercent =
      genre.total_rounds > 0 ? ((genre.total_rounds - genre.wins) / genre.total_rounds) * 100 : 0;
    return {
          ...
          <ResponsiveContainer width="100%" height={300}>
            <RadarChart data={chartData}>
              ...
              <PolarRadiusAxis
                ...
                tick={{ fill: '#9ca3af', fontSize: 9 }}
              />
              <Radar name="Wins" dataKey="wins" stroke="#3b82f6" strokeWidth={2} fillOpacity={0} />
              <Radar
                name="Losses"
                dataKey="losses"
                stroke="#ef4444"
