...
export const ScoreDisplay: React.FC<ScoreDisplayProps> = ({
  ...
  const eloRatingDisplay = eloRating !== undefined ? (
    <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={transitions.spring} className="flex items-center justify-center gap-2 mb-3 px-2 py-1 bg-primary/10 rounded border border-primary/30" data-testid="elo-rating">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {hasWon && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={transitions.springBouncy} className="flex items-center gap-1 text-yellow-400">
      ...
      <div className="text-center mb-4">
        <CountUp value={scoreInfo.score} className={`text-3xl font-bold ${hasWon ? 'text-yellow-400' : 'text-white'}`} />
      ...
      <div className="space-y-2 mb-4">
        ...
        {scoreInfo.bonuses.map((bonus, index) => (
          <motion.div key={index} initial={{ x: 10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: index * 0.1, ...transitions.smooth }} className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2 text-muted-foreground">
      ...
      <div className="mt-3 flex gap-2">
        {hasMultiLineBonus && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={transitions.spring} className="px-2 py-1 bg-primary/10 rounded text-xs text-primary border border-primary/30">
            Multi-Line
          </motion.div>
        )}
        {hasSpeedBonus && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={transitions.spring} className="px-2 py-1 bg-green-500/20 rounded text-xs text-green-400 border border-green-500/30">
            Speed
          </motion.div>
        )}
        {scoreInfo.lines.length >= 2 && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={transitions.spring} className="px-2 py-1 bg-yellow-500/20 rounded text-xs text-yellow-400 border border-yellow-500/30">
            Dominating
          </motion.div>
        )}
      </div>
